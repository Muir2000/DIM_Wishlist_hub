...
핵심 불변식:
  ...
  * 퍼크 해시는 Inventory Item 플러그 해시. 강화(Enhanced) 퍼크는 emit 하지 않고
    기본(base) 해시만 출력한다(DIM 이 강화판을 자동 매칭).
  * 트래시롤 = 아이템 해시 음수화. 와일드카드 = item=-69420 (desirable 전용).
...
@dataclass
class RollRequest:
    """단일 롤(무기 1종에 대한 원하는/원치 않는 퍼크 조합) 요청.
    ...
    # 굴릴 수 없는 퍼크 조합 줄은 생략(죽은 줄 방지). 비우면 필터 없이 그대로 emit.
...
def compile_roll(
    ...
    if roll.wildcard and roll.trash:
        ...
        raise CompileError("와일드카드와 트래시롤은 동시에 사용할 수 없습니다.")
    ...
    if n_combos > MAX_ROLL_COMBOS:
        raise CompileError(
            f"한 롤의 퍼크 조합이 너무 많습니다({n_combos}). 열당 선택 수를 줄이세요(최대 {MAX_ROLL_COMBOS} 조합)."
        )
...
def _filter_columns_for_variant(ordered_cols, pool):
    """변형 무기 풀(pool)에 맞춰 각 열의 퍼크를 거른다.

    - pool 이 None 이면 필터 없이 그대로(원본 반환).
    - 제약된 열 중 하나라도 변형이 굴릴 수 있는 퍼크가 0개면 None(이 변형은 스킵).
...
def parse_wishlist(text: str) -> dict:
    ...
    - 같은 블록(연속된 같은 주석 하 같은 item_hash·트래시/와일드카드)의 여러 줄은 **하나의 롤로
      합친다**(퍼크 합집합) — DIM 의 카르테시안 전개를 역으로 묶어 멀티선택 롤로 복원.
    - 빈 줄은 블록 노트 종료.
